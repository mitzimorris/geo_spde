...
Geolocated data where we expect measurements at nearby locations to be similar to each other,
but not necessarily similar to distant locations, can be modeled using a Gaussian process model
which is parameterized by a mean function and covariance function.
But when the number of observed locations is greater than $1000$ this approach is too computationally expensive for Stan,
...
For large geospatial datasets the best alternatives are R-INLA or TMB.
The key insight that allows R-INLA to fit GPs comes from
...
By transforming the Gaussian process into a Gaussian Markov random field (GMRF),
...
It is important to note that R-INLA is still much faster because
its implementations are optimized for sparse precision matrices
...
- It enables the use of sparse matrix algorithms with O(n^(3/2)) complexity in 2D.
